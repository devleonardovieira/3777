	...
	int32_t index, Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/)
{
	...
	if(item->isStackable())
	{
		if(toItem && toItem->getID() == item->getID())
		{
			...
			{
				n = std::min((uint32_t)5000 - toItem->getItemCount(), m);
...
void Game::addMoney(Cylinder* cylinder, int64_t money, uint32_t flags /*= 0*/)
{
	...
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		do
		{
			Item* remainItem = Item::CreateItem(it->second, std::min((int64_t)5000, tmp));
...
bool Game::playerPurchaseItem(uint32_t playerId, uint16_t spriteId, uint16_t count, uint16_t amount,
	bool ignoreCap/* = false*/, bool inBackpacks/* = false*/)
{
	...
	uint16_t subType = count;
	if(it.isFluidContainer() && count < uint16_t(sizeof(reverseFluidMap) / sizeof(int16_t)))
...
bool Game::playerSellItem(uint32_t playerId, uint16_t spriteId, uint16_t count, uint16_t amount)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
...
bool Game::playerLookInShop(uint32_t playerId, uint16_t spriteId, uint16_t count)
{
	Player* player = getPlayerByID(playerId);
	if(player == NULL || player->isRemoved())
		return false;
	...
	if(it.isFluidContainer())
	{
		if(subType == 3) // FIXME: hack
			subType = 11;
		else if(count < uint16_t(sizeof(reverseFluidMap) / sizeof(int16_t)))
			subType = reverseFluidMap[count];
	}
	...
	MagicEffect_t hitEffect/* = MAGIC_EFFECT_UNKNOWN*/, Color_t hitColor/* = COLOR_UNKNOWN*/, bool force/* = false*/)
{
	...
	if(healthChange > 0)
	{
		...
		{
			char buffer[20];
			if (healthChange < 1000) {
				sprintf(buffer, "+%d", healthChange); // Deixa a mudan̤a de sa̼de normal se for menor que 1000
			} else {
				double healthChangeInK = healthChange / 1000.0;
				sprintf(buffer, "+%.2fk", healthChangeInK); // Exibe a mudan̤a de sa̼de em formato de n̼mero com casas decimais para milhares
			}
		...
		if(damage != 0)
		{
			if(target->hasCondition(CONDITION_MANASHIELD) && combatType != COMBAT_UNDEFINEDDAMAGE)
			{
				...
				if(manaDamage != 0)
				{
					...
					char buffer[20];
					if (manaDamage < 1000) {
						sprintf(buffer, "%d", manaDamage); // Deixa a mudan̤a de mana normal se for menor que 1000
					} else {
						double manaChangeInK = manaDamage / 1000.0;
						sprintf(buffer, "%.2fk", manaChangeInK); // Exibe a mudan̤a de mana em formato de n̼mero com casas decimais para milhares
					}
			...
			if(damage > 0)
			{
				...
				switch(combatType)
				{
					...
					case COMBAT_MANADRAIN:
					{
						...
						magicEffect = MAGIC_EFFECT_WRAPS_BLUE;
						break;
					}
				...
				if(textColor < COLOR_NONE && magicEffect < MAGIC_EFFECT_NONE)
				{
					char buffer[20];
					if (damage < 1000) {
						sprintf(buffer, "%d", damage); // Deixa o dano normal se for menor que 1000
					} else {
						double damageInK = damage / 1000.0;
						sprintf(buffer, "%.2fk", damageInK); // Exibe o dano em formato de n̼mero com casas decimais para milhares
					}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange)
{
	...
	else
	{
		...
		if(manaLoss > 0)
		{
			...
			char buffer[20];
			if (manaLoss < 1000) {
				sprintf(buffer, "%d", manaLoss); // Deixa a perda de mana normal se for menor que 1000
			} else {
				double manaLossInK = manaLoss / 1000.0;
				sprintf(buffer, "%.2fk", manaLossInK); // Exibe a perda de mana em formato de n̼mero com casas decimais para milhares
			}
...
void Game::checkLight()
{
	...
	switch(lightState)
	{
		...
		{
			newLightLevel += (LIGHT_LEVEL_DAY - LIGHT_LEVEL_DAY);
			lightChange = false;
			break;
		}
